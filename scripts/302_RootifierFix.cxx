...
int detect_daqh_offset(std::string &_data_line, unsigned char _start_4_bits, unsigned char _end_4_bits);
                        ...
                        if (valid_line_pool.size() >= target_SWTFC_pool_size || input_file_is_last_chunk) {
                            for (int i = 0; i < valid_line_pool.size() - _SWTFC_window_size; i++) {
                                ...
                                if (_match_line_counter == LINE_NUMBER_PER_FPGA){
                                    ...
                                    // sort the _match_line_index by the line ID
                                    std::sort(_match_line_index.begin(), _match_line_index.end(), [&valid_line_pool](int a, int b) {
                                        return (unsigned char) valid_line_pool.at(a).at(3) < (unsigned char) valid_line_pool.at(b).at(3);
                                    });
                                    ...
                                    for (auto &index : _match_line_index) {
                                        ...
                                        if (_line_id == 0x00) {
                                            _offset_bits = detect_daqh_offset(valid_line_pool.at(index), 0x5, 0x5);
                                        }
                                        switch (_line_id) {
                                            case 0x00:
                                            {
...
int detect_daqh_offset(std::string &_data_line, unsigned char _start_4_bits, unsigned char _end_4_bits){
// UInt_t _daqh = (unsigned char) valid_line_pool.at(index).at(8) << 24 | (unsigned char) valid_line_pool.at(index).at(9) << 16 | (unsigned char) valid_line_pool.at(index).at(10) << 8 | (unsigned char) valid_line_pool.at(index).at(11);
    auto _daqh_byte_left_half  = (unsigned char)_data_line.at(8) >> 4;
    auto _daqh_byte_right = (unsigned char)_data_line.at(11) & 0x0f;
    if (_daqh_byte_left_half == _start_4_bits && _daqh_byte_right == _end_4_bits) {
        return 0; // no offset
    }
    // otherwise, slip the string by 1 and 2 bits left and right
    auto _daqh_byte_left_half_shifted_1_left  = (unsigned char)_data_line.at(8) >> 5 | ((unsigned char)_data_line.at(7) & 0x01) << 3;
    auto _daqh_byte_right_shifted_1_left = ((unsigned char)_data_line.at(11) >> 1) & 0x0f;
    if (_daqh_byte_left_half_shifted_1_left == _start_4_bits && _daqh_byte_right_shifted_1_left == _end_4_bits) {
        LOG(INFO) << "DAQH offset found: 1 bit left";
        return -1; // offset 1 bit left
    }
    auto _daqh_byte_left_half_shifted_2_left  = (unsigned char)_data_line.at(8) >> 6 | ((unsigned char)_data_line.at(7) & 0x03) << 2;
    auto _daqh_byte_right_shifted_2_left = ((unsigned char)_data_line.at(11) >> 2) & 0x0f;
    if (_daqh_byte_left_half_shifted_2_left == _start_4_bits && _daqh_byte_right_shifted_2_left == _end_4_bits) {
        LOG(INFO) << "DAQH offset found: 2 bits left";
        return -2; // offset 2 bits left
    }
    auto _daqh_byte_left_half_shifted_1_right = ((unsigned char)_data_line.at(8) >> 3) & 0x0f;
    auto _daqh_byte_right_shifted_1_right = ((unsigned char)_data_line.at(11) << 1) & 0x0f | ((unsigned char)_data_line.at(12) >> 7) & 0x01;
    if (_daqh_byte_left_half_shifted_1_right == _start_4_bits && _daqh_byte_right_shifted_1_right == _end_4_bits) {
        LOG(INFO) << "DAQH offset found: 1 bit right";
        return -1; // offset 1 bit right
    }
    auto _daqh_byte_left_half_shifted_2_right = ((unsigned char)_data_line.at(8) >> 2) & 0x0f;
    auto _daqh_byte_right_shifted_2_right = ((unsigned char)_data_line.at(11) << 2) & 0x0f | ((unsigned char)_data_line.at(12) >> 6) & 0x03;
    if (_daqh_byte_left_half_shifted_2_right == _start_4_bits && _daqh_byte_right_shifted_2_right == _end_4_bits) {
        LOG(INFO) << "DAQH offset found: 2 bits right";
        return -2; // offset 2 bits right
    }
    LOG(WARNING) << "DAQH offset not found! Start bits: " << (int)_start_4_bits << ", End bits: " << (int)_end_4_bits << ", DAQH left half: " << (int)_daqh_byte_left_half << ", DAQH right: " << (int)_daqh_byte_right;
    // print the line in hex
    std::stringstream ss;
    for (size_t i = 0; i < _data_line.size(); i++) {
        ss << std::hex << std::setw(2) << std::setfill('0') << (int)(unsigned char)_data_line.at(i) << " ";
    }
    LOG(ERROR) << "Data line: " << ss.str();
    return 0; // no offset found, return 0
}
